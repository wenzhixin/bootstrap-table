(function (global, factory) {
  ...
  var UtilsFilterControl = {
    ...
    addOptionToSelectControl: function addOptionToSelectControl(selectControl, _value, text) {
      var value = _jquery2.default.trim(_value);
      var $selectControl = (0, _jquery2.default)(selectControl.get(selectControl.length - 1));
      if (!UtilsFilterControl.existOptionInSelectControl(selectControl, value)) {
        $selectControl.append((0, _jquery2.default)('<option></option>').attr('value', value).text((0, _jquery2.default)('<div />').html(text).text()));
      }
    },
    sortSelectControl: function sortSelectControl(selectControl) {
      ...
      $opts.sort(function (a, b) {
        var aa = (0, _jquery2.default)(a).text().toLowerCase();
        var bb = (0, _jquery2.default)(b).text().toLowerCase();
        if (_jquery2.default.isNumeric(a) && _jquery2.default.isNumeric(b)) {
    ...
    getCursorPosition: function getCursorPosition(el) {
      if (Utils.isIEBrowser()) {
        if ((0, _jquery2.default)(el).is('input[type=text]')) {
          var pos = 0;
    ...
    setCursorPosition: function setCursorPosition(el) {
      (0, _jquery2.default)(el).val(el.value);
    },
    copyValues: function copyValues(that) {
      ...
      header.find(searchControls).each(function () {
        that.options.valuesFilterControl.push({
          field: (0, _jquery2.default)(this).closest('[data-field]').data('field'),
          value: (0, _jquery2.default)(this).val(),
          position: UtilsFilterControl.getCursorPosition((0, _jquery2.default)(this).get(0)),
          hasFocus: (0, _jquery2.default)(this).is(':focus')
        });
      });
    },
    setValues: function setValues(that) {
      ...
      if (that.options.valuesFilterControl.length > 0) {
        ...
        header.find(searchControls).each(function (index, ele) {
          field = (0, _jquery2.default)(this).closest('[data-field]').data('field');
          result = _jquery2.default.grep(that.options.valuesFilterControl, function (valueObj) {
            return valueObj.field === field;
          });

          if (result.length > 0) {
            (0, _jquery2.default)(this).val(result[0].value);
            if (result[0].hasFocus) {
              ...
              }((0, _jquery2.default)(this).get(0), result[0].position);
            }
          }
        });
    ...
    collectBootstrapCookies: function collectBootstrapCookies() {
      ...
      if (foundCookies) {
        _jquery2.default.each(foundCookies, function (i, _cookie) {
          var cookie = _cookie;
          if (/./.test(cookie)) {
            cookie = cookie.split('.').pop();
          }

          if (_jquery2.default.inArray(cookie, cookies) === -1) {
            cookies.push(cookie);
          }
        });
        return cookies;
    ...
    initFilterSelectControls: function initFilterSelectControls(that) {
      ...
      _jquery2.default.each(that.header.fields, function (j, field) {
        var column = that.columns[that.fieldsColumnsIndex[field]];
        var selectControl = (0, _jquery2.default)('.bootstrap-table-filter-control-' + UtilsFilterControl.escapeID(column.field));
    ...
    createControls: function createControls(that, header) {
      ...
      _jquery2.default.each(that.columns, function (i, column) {
        ...
        if (column.filterData !== undefined && column.filterData.toLowerCase() !== 'column') {
          ...
          if (filterDataType !== null) {
            ...
            selectControl = (0, _jquery2.default)('.bootstrap-table-filter-control-' + UtilsFilterControl.escapeID(column.field));
      ...
      if (addedFilterControl) {
        header.off('keyup', 'input').on('keyup', 'input', function (event, obj) {
          ...
          if (_jquery2.default.inArray(event.keyCode, [37, 38, 39, 40]) > -1) {
            return;
          }
        ...
        if (header.find('.date-filter-control').length > 0) {
          _jquery2.default.each(that.columns, function (i, _ref6) {
            ...
            if (filterControl !== undefined && filterControl.toLowerCase() === 'datepicker') {
              header.find('.date-filter-control.bootstrap-table-filter-control-' + field).datepicker(filterDatepickerOptions).on('changeDate', function (_ref7) {
                ...
                (0, _jquery2.default)(Utils.sprintf('#%s', currentTarget.id)).val(currentTarget.value);
                // Fired the keyup event
                (0, _jquery2.default)(currentTarget).keyup();
              });
            }
          });
        }
  ...
  _jquery2.default.extend(_jquery2.default.fn.bootstrapTable.columnDefaults, {
  ...
  _jquery2.default.fn.bootstrapTable.methods.push('triggerSearch');
    ...
    _createClass(_class, [{
      ...
      value: function initToolbar() {
        ...
        if (this.options.filterControl && this.options.filterShowClear) {
          ...
          if (!$btnClear.length) {
            $btnClear = (0, _jquery2.default)([Utils.sprintf('<button class="btn btn-%s filter-show-clear" ', this.options.buttonsClass), Utils.sprintf('type="button" title="%s">', this.options.formatClearFilters()), Utils.sprintf('<i class="%s %s"></i> ', this.options.iconsPrefix, this.options.icons.clear), '</button>'].join('')).appendTo($btnGroup);

            $btnClear.off('click').on('click', _jquery2.default.proxy(this.clearFilterControl, this));
          }
        }
      }
    }, {
      ...
      value: function initSearch() {
        var that = this;
        var fp = _jquery2.default.isEmptyObject(that.filterColumnsPartial) ? null : that.filterColumnsPartial;
          ...
          Object.keys(item).forEach(function (key, index) {
            ...
            if (fval === '') {
              ...
              if (thisColumn && thisColumn.searchFormatter) {
                value = _jquery2.default.fn.bootstrapTable.utils.calculateObjectValue(that.header, that.header.formatters[_jquery2.default.inArray(key, that.header.fields)], [value, item, i], value);
              }

              if (_jquery2.default.inArray(key, that.header.fields) !== -1) {
      ...
      value: function onColumnSearch(event) {
        ...
        UtilsFilterControl.copyValues(this);
        var text = _jquery2.default.trim((0, _jquery2.default)(event.currentTarget).val());
        var $field = (0, _jquery2.default)(event.currentTarget).closest('[data-field]').data('field');
      ...
      value: function clearFilterControl() {
        if (this.options.filterControl && this.options.filterShowClear) {
          ...
          _jquery2.default.each(that.options.valuesFilterControl, function (i, item) {
          ...
          timeoutId = setTimeout(function () {
            if (cookies && cookies.length > 0) {
              _jquery2.default.each(cookies, function (i, item) {
          ...
          if (controls.length > 0) {
            this.filterColumnsPartial = {};
            (0, _jquery2.default)(controls[0]).trigger(controls[0].tagName === 'INPUT' ? 'keyup' : 'change', { keyCode: 13 });
          } else {
            return;
          }
          ...
          if (that.options.sortName !== table.data('sortName') || that.options.sortOrder !== table.data('sortOrder')) {
            var sorter = header.find(Utils.sprintf('[data-field="%s"]', (0, _jquery2.default)(controls[0]).closest('table').data('sortName')));
            if (sorter.length > 0) {
              that.onSort({ type: 'keypress', currentTarget: sorter });
              (0, _jquery2.default)(sorter).find('.sortable').trigger('click');
            }
          }
        }
      }
