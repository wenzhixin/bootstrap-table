 ...
 */
 ...
 (function ($) {
    ...
    var escapeID = function(id) {
        return convertToSlug(String(id).replace( /(:|\.|\[|\]|,)/g, "\\$1" ));
    };
            ...
            if (header.find('.date-filter-control').length > 0) {
                $.each(that.columns, function (i, column) {
                    if (column.filterControl !== undefined && column.filterControl.toLowerCase() === 'datepicker') {
                        header.find('.date-filter-control.bootstrap-table-filter-control-' + escapeID(column.field)).datepicker(column.filterDatepickerOptions)
        ...
        filterTemplate: {
            ...
            select: function (that, field, isVisible) {
                ...
                return sprintf('<select class="form-control bootstrap-table-filter-control-%s" style="width: 100%; visibility: %s" dir="%s"></select>',
                    escapeID(field), isVisible, getDirectionOfSelectOptions(that.options.alignmentSelectControlOptions));
            },
