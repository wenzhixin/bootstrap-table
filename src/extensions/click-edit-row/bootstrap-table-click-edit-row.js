        ...
        if(table.editing){
            ...
            table.columns.forEach(function(column, i){
                ...
                switch(column.editable){
                    case 'input':
                        var div=$('<div class="editable-input" data-fieldName='+column.field+'/>');
                        txt.push(tarNode.find('td').eq(column.fieldIndex).text());
                        div.append($('<div class="col-md-6 col-sm-6 col-xs-6"><input type="text" class="form-control"/></div>'));
                        div.append($('<span class="clear-selector col-sm-2 control-label vl-align"><i class="fa fa-times-circle-o" aria-hidden="true"></i></span>'));
                        tarNode.find('td').css('position', 'relative').eq(column.fieldIndex).text('').append(div);
                        break;
                    case 'select':
            ...
            tarNode.find('td > div').last().append('<div id="tooling" class="submitbtn editable-buttons"/>');
            $('.clear-selector').on('click', function(){ $(this).parent().find('input').val('');});
