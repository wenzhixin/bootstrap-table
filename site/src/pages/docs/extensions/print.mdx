...
- **Detail:**
  ...
  Receive HTML `<table>` string and `styles` string as parameters, returns HTML string for printing. This option is used for styling and adding a header or footer.
...
The print extension supports `rowStyle` and `cellStyle` options to apply styles to rows and cells in the printed table. Both the `css` property (inline styles) and the `classes` property are applied to the printed output.

The `css` property supports multiple formats: plain objects (`{ color: 'red' }`), strings (`'color: red'`), and arrays (`['color: red', { 'font-weight': 'bold' }]`), matching the behavior of the core table renderer.

If you need class-based styling in the print output, use the `printStyles` option to include the relevant CSS file. Note that if you use a custom `printPageBuilder`, you must include the `styles` parameter (the second argument, an HTML string containing `<link>` tags) in your returned HTML to ensure the CSS links are injected into the print window.
